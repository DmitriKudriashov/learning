...
class Interface < BaseMenu
  ...
  def create_new_route
    loop do
      ...
      new_route = new_route_creating(first_station, last_station)
      new_route_check_add_to_all (new_route)  unless new_route.nil?
      gets #pause
    end # loop
  end
  ...
  def new_route_check_add_to_all(new_route)
    new_route_string = "Route: #{new_route.first_station.name} -> #{new_route.last_station.name}"
